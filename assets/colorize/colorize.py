...
def colorize_image(input_path: str, output_path: str, model_path: str, render_factor: int = 19) -> None:
    ...
    流程:
    1. 读取原图，提取 L 通道（亮度）
    2. 将 L 通道缩放到 256x256，归一化后送入 ONNX 模型推理
    3. 模型输出上色后的 RGB 图像（256x256）
    4. 从输出中提取颜色（AB 通道），缩放回原始尺寸
    5. 用原始 L 通道 + 推理得到的 AB 通道重建彩色图像（保留原始清晰度）
    6. 保存结果
    """
    # 读取原图
    ...
    if output_data.ndim == 3 and output_data.shape[-1] == 3:
        # 输出是 RGB，转为 LAB 提取 AB
        out_L = rgb_to_lab_l(colorized)
