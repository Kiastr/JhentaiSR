...
class SettingReadPage extends StatelessWidget {
  ...
  Widget _buildAnime4KGroup(BuildContext context) {
    return Column(
      children: [
        SwitchListTile(
          title: const Text('Anime4K 超分辨率'),
          subtitle: const Text('使用 Anime4K 算法实时放大图片'),
          value: readSetting.enableAnime4K.value,
          onChanged: readSetting.saveEnableAnime4K,
        ),
        if (readSetting.enableAnime4K.value) ...[
          SwitchListTile(
            title: const Text('网络图片也进行超分'),
            value: readSetting.enableAnime4KForNetwork.value,
            onChanged: readSetting.saveEnableAnime4KForNetwork,
          ).fadeIn(),
          ListTile(
            title: const Text('放大倍数'),
            trailing: DropdownButton<double>(
              value: readSetting.anime4KScaleFactor.value,
              onChanged: (v) => readSetting.saveAnime4KScaleFactor(v!),
              items: [1.5, 2.0, 3.0, 4.0]
                  .map((e) => DropdownMenuItem(child: Text('${e}x'), value: e))
                  .toList(),
            ),
          ).fadeIn(),
          ListTile(
            ...
            trailing: DropdownButton<double>(
              value: readSetting.anime4KPushStrength.value,
              onChanged: (v) => readSetting.saveAnime4KPushStrength(v!),
              items: [0.0, 0.15, 0.31, 0.5, 0.75]
          ...
          ListTile(
            ...
            trailing: DropdownButton<double>(
              value: readSetting.anime4KPushGradStrength.value,
              onChanged: (v) => readSetting.saveAnime4KPushGradStrength(v!),
              items: [0.0, 0.5, 1.0, 1.5, 2.0]
                  .map((e) => DropdownMenuItem(child: Text(e.toString()), value: e))
                  .toList(),
            ),
          ).fadeIn(),
