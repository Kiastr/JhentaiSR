...
namespace ColorfulSoft.DeOldify
{
    ...
    internal static unsafe class DeOldify
    {
        ...
        ///<summary>
        /// Initializes DeOldify parameters from an external model file.
        ///</summary>
        ///<param name="modelPath">Path to the model file (.model or .hmodel).</param>
        public static void Initialize(string modelPath = null)
        {
            ...
            #if half
                #if stable
                    stream = Assembly.GetExecutingAssembly().GetManifestResourceStream("Stable.hmodel");
                #else
                    stream = Assembly.GetExecutingAssembly().GetManifestResourceStream("Artistic.hmodel");
                #endif
            #else
                #if stable
                    stream = Assembly.GetExecutingAssembly().GetManifestResourceStream("Stable.model");
                #else
                    stream = Assembly.GetExecutingAssembly().GetManifestResourceStream("Artistic.model");
                #endif
            #endif
                if(stream == null)
                {
                    throw new FileNotFoundException(
                        "Model file not found. Either provide a valid modelPath or compile with embedded model resource. " +
                        "Expected model file at: " + (modelPath ?? "(null)"));
                }
